...
global $content_width; 
	...
	add_image_size('fashionbuzz-homepage-thumb',240,145,true);
	register_nav_menus( array(
		'primary' => __( 'Primary Menu', 'fashionbuzz' ),
	) );
	add_theme_support( 'custom-background', array(
		'default-image' => get_template_directory_uri().'/images/back-bg.jpg',
	) );
...
function fashionbuzz_widgets_init() {
	...
	register_sidebar( array(
		'name'          => __( 'Page Sidebar', 'fashionbuzz' ),
...
function fashionbuzz_font_url(){
		...
		*/
		$ptsans = _x('on', 'PT Sans font:on or off','fashionbuzz');
		
		/* Translators: If there are any character that are
		* not supported by Merienda One, translate this to off, do not
		* translate into your own language.
		*/
		$merienda = _x('on', 'Merienda One font:on or off','fashionbuzz');
		
		/* Translators: If there are any character that are
		* not supported by Oswald, translate this to off, do not
		* translate into your own language.
		*/
		$oswald = _x('on', 'Oswald font:on or off','fashionbuzz');
		
		/* Translators: If there are any character that are
		* not supported by Roboto Condensed, translate this to off, do not
		* translate into your own language.
		*/
		$roboto = _x('on', 'Roboto Condensed font:on or off','fashionbuzz');
		
		if('off' !== $ptsans || 'off' !==  $merienda || 'off' !==  $oswald || 'off' !==  $roboto){
			$font_family = array();
			...
			if('off' !== $merienda){
				$font_family[] = 'Merienda One:400,700';
			}
...
function fashionbuzz_scripts() {
	wp_enqueue_style( 'fashionbuzz-font', fashionbuzz_font_url(), array() );
	...
	wp_enqueue_style( 'fashionbuzz-bootstrap-style', get_template_directory_uri().'/css/bootstrap.css' );
	wp_enqueue_script( 'fashionbuzz-bootstrap-script', get_template_directory_uri().'/js/bootstrap.js' );
...
function fashionbuzz_ie_stylesheet(){
	global $wp_styles;
	...
	*/
	wp_enqueue_style('fashionbuzz-ie', get_template_directory_uri().'/css/ie.css', array('fashionbuzz-style'));
	$wp_styles->add_data('fashionbuzz-ie','conditional','IE');
	}
add_action('wp_enqueue_scripts','fashionbuzz_ie_stylesheet');
...
define('pro_theme_url','http://flythemes.net/wordpress-themes/fashionbuzz-wordpress-theme/');
define('theme_doc','http://flythemesdemo.net/documentation/fashionbuzz-doc/');
...
function fashionbuzz_credit_link(){
